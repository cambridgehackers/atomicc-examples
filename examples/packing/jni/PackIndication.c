...
const uint32_t PackIndication_reqinfo = 0x1000c;
const char * PackIndication_methodSignatures()
{
...
int PackIndication_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    PackIndicationData tempdata __attribute__ ((unused));
    memset(&tempdata, 0, sizeof(tempdata));
    volatile unsigned int* temp_working_addr = p->transport->mapchannelInd(p, channel);
    switch (channel) {
    case CHAN_NUM_PackIndication_heard: {
        ...
        ((PackIndicationCb *)p->cb)->heard(p, tempdata.heard.v, tempdata.heard.writeCount, tempdata.heard.readCount, tempdata.heard.seqno);
      } break;
    default:
        PORTAL_PRINTF("PackIndication_handleMessage: unknown channel 0x%x\n", channel);
        if (runaway++ > 10) {
            PORTAL_PRINTF("PackIndication_handleMessage: too many bogus indications, exiting\n");
#ifndef __KERNEL__
            exit(-1);
#endif
        }
